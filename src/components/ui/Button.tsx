...
export default function Button({ children, variant='primary', icon, className, ...props }: Props) {
  const base = 'inline-flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-150'
  const map:Record<string,string> = {
    primary: 'bg-[var(--color-primary)] text-white shadow-md hover:brightness-95',

...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const seed: Conversation = { id: uid(), title: 'Bienvenue sur Aura IA', folderId: null, messages: [{ id: uid(), role:'assistant', text: 'Bonjour — je suis Aura IA. Pose ta première question.', createdAt: now() }], createdAt: now(), updatedAt: now() }
  ...
  function createFolder(title:string) {
    const f: Folder = { id: uid(), title: title || 'Dossier', createdAt: now() }
    setFolders(prev => [f, ...prev])
    return f
  }
  function createConversation(title?:string, folderId?:string|null) {
    const c: Conversation = { id: uid(), title: title || 'Nouvelle conversation', folderId: folderId ?? null, messages: [], createdAt: now(), updatedAt: now() }
    setConversations(prev => [c, ...prev])
    setActiveConversationId(c.id)
    return c
  }
  function setActiveConversation(id?:string|null){ setActiveConversationId(id ?? null) }
  function addMessage(convId:string, role:Role, text:string) {
    setConversations(prev => prev.map(c => c.id === convId ? { ...c, messages: [...c.messages, { id: uid(), role, text, createdAt: now() }], updatedAt: now() } : c))
  }
  ...
  function clearAll() {
    setConversations([]); setFolders([]); setActiveConversationId(undefined); localStorage.removeItem(KEY)
  }
